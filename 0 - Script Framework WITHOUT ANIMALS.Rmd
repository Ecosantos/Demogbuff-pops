...
# USE TO CLEAN ENVIRONMENT TIME TO TIME
# rm(list=ls())
```
...
-   Intermediary code and data include:
    ...
    -   4 - Analyses with MCMCglmm
        -   It includes a separate model selection analyses to decide
            the most plausible model to use along the analyses
    -   5 - Core function: Stochastic elasticities of variance

> Animals were removed from analyses because data are skewed and we
> concluded results are not robust enough to compare to plants. However,
> for those interested in running the analyses with animals included
> just need to follow the same steps for plants as life history traits
> and climatic variables are available for animals in the intermediary
> dataset produced it means `LHtraits.RDS` and `climate_df`.
> <!-- Animais foram removidos das análises a partir desta versão (11/abril/2024) no entanto, todos os dados necessários para rodar as análises com animas permanecerão disponíveis -->
...
AnimalsID<- filter(CleanData$Metadata,Kingdom=="Animalia")$ID
MetadataClean<-Metadata<-CleanData$Metadata%>%filter(Kingdom=="Plantae")
...
-   Produce:
    ...
    -   Life history traits include:
        -   `D`: Reproductive window ($L$)
            -   Temporal spread of reproduction throughout life span as
                quantified by @demetrius1974 's entropy ($S$).
                -   High/low $S$ values correspond to
                    iteroparous/semelparous populations
        -   `La`: Mean age at first reproduction ($L_a$)
            -   Average amount of time from birth to reproduction
        -   `LaProp`: Probability of an individual become reproductive
            along their lifetime ($p_a$)
        -   `e`: Mean life expectancy ($\eta _e$)
        -   `growth`: individual development ($\gamma$)
        -   `H`: Distribution of mortality risk along the life cycle
            ($H$)
    -   **`mahal.dist`** The mahalanobis distance
    -   `is.outlier`: TRUE or FALSE to indicate outliers detected with
...
```{r "life-history PCA", fig.width=16}
LHpca<-spLHmat[!c(rownames(spLHmat) %in% AnimalsID),] %>% # Make sure animals are not included in any step
  filter(complete.cases(.))%>%PCA(.,graph=FALSE,scale=TRUE) # Using raw data without imputatation
...
Extract and store climatic data from CHELSAcruts (@karger2017) in an
intermediary dataset, remove collinear variable, perform environmental
PCA and plot PCA's axes across the globe
...
a dedicated Jupyter notebook to run on Google Colab is provided:
...
3 - `Climatic variables calculation.R` use the already extracted
climatic data to:

-   Produce data summary and extract metrics described in methods
...
Run environmental PCA and show its output

```{r, fig.width=16}
# Produce the climatic/environmental PCA
ClimPCA<-climate_df[!c(rownames(climate_df) %in% AnimalsID),]%>%
...
```{r message=FALSE, warning=FALSE, fig.width=16, fig.height=6}
...
-   This script creates the following functions:
    ...
    1.  `My.vitalRatePerturbation`: produce the stochastic elasticity
        with respect to variance $| \sum E_v^\sigma |$

        -   This function is an adaptation of `VitalRatePerturbation`
            function in the `Rage` package (see @jones2022)

    2.  `array_to_matrix`: an ancillary function to transform timeseries
        of MPMs, usually returned as an array element to a timeserie of
        MPM in a object class matrix.
...
## 5.2. Derivative demographic buffering in plants

-   Derivates demographic buffering in plants 
-   Merge into a single data frame

<!-- COMMENTARY: 
ElasSigVR_data and StochElasVR_data are simpler versions created without standard deviation because sd was not included in the GLMM model. However, the full version ("ElasSigVR_full") is still required to summary
statistics below in .-->
...
- Buffering calculation is saved in a separate object
...
merged_data<-databuff_data[!c(databuff_data$ID %in% AnimalsID),]%>%
...
-   Check must return:
  -   No duplicates: Each species is a tip in the phylogeny
  -   No polytomyes: Methods require no polytomyes so it need to be
      removed
  -   Is.rooted == TRUE: A phylogeny need a root, it is, an initial
      state
  -   is.binary == TRUE:
  -   is.ultrametric == TRUE: Ultrametic means branches lenght reflect
...
Data_model now includes absolute values of sum of stochastic elasticity
within respect to variance $| \sum E_v^\sigma |$ (i.e,
cumulativa_SigEla).[^1]
...
Save the data for model selection and inference analyses
```{r eval=FALSE, include=TRUE}
save(data_model,subtree_Plants, file = "Data/GLMMdata.Rdata")
```
...
With each element created separated we can run a loop with a generic
model where `model(variable ~ fixEffect)`
...
it take a long time to run in a in-house computer. Moreover, a long
script was created `5b - MCMCglmm run.R`. Thus, to accelerate the model
performance and save time we also created a jupyter notebook integrated
with Google Collab `5b - MCMCglmm run.ipynb`.

Most importantly, a partial output was created with the GLMM models in
`Data/MCMCglmm_output.rds`.

Next steps use this partial output to keep running the analyses.
...
Rank more and less buffered populations. Again sum of stochastic
elasticity within respect to variance $| \sum E_v^\sigma |$ is used.
...
ToRank%>%
left_join(.,ElasSigVR_full,by="ID")%>%mutate_if(is.numeric,round,3)

```

<!-- REMOVED BECAUSE THERE IS NO LONGER ANIMALS IN THE ANALYSES 
## 10.2. Animals and plants overlaps?

A simple t-test was used and the answer is **Yes!**
...
An accessory function is necessary to better represent very small
numbers.
...
Quantify proportional contribution of each vital rate to
$| \sum E_v^\sigma |$. We used percentage `RelContrib*100` to improve
readability.
...
left_join(.,MetadataClean%>%select(-lambda, -mat)%>%distinct(),by="ID")

RelContrib%>%mutate_if(is.numeric,round,5)
...
summarise(Relativemean=mean(RelativeProp),   #Relative contribution (mean for each kingdom)
		SD=sd(RelativeProp),
		n=n())

RelContrib_df%>%
  mutate(Relativemean=Relativemean*100,
         SD=SD*100)%>%mutate_if(is.numeric,round,3)
```
...
<!--#TODO: EDITAR E TROCAR FIGURA ASSIM QUE FICAR PRONTA-->

![](Figures%20edited/Fig2%20plants%20-%20Vital%20rate%20contribution_editada.png){width="500"}

#### Plot Elasticities

Plot shows stochastic elasticities of each vital rate (A) and its
relative effect on $\lambda_{s}$, it means $E_{v}^\sigma$
...
plotbuff_sigElas$data<-plotbuff$data%>%
  filter(Form=="SigElas")

cowplot::plot_grid(ncol=1,labels="AUTO",
                   # Vital rate elasticity
                   plotbuff_base+
                     ylab(expression(paste("Vital rate elasticity ( ",E[v]," )"))),
                   # Sum of stochastic elasticity with respect to variance
                   plotbuff_sigElas+
                     ylab(bquote("Effect of environmental variation ("~ sum(E[v]^sigma)~" )")))
...
RelContrib_plot<-rbind(
  ...
  geom_pointrange(position=position_dodge(0.4),
                  aes(ymin = pmax(Relativemean-SD, 0), ymax = pmin(Relativemean+SD, 1),
                      fill=Taxa,shape=Kingdom, group=Taxa,color=IsMean),linewidth = 1,size= 1.1,alpha=.9)+
  ylab(NULL)+xlab(NULL)+
  scale_shape_manual(values=c(23,21))+
  scale_color_manual(values=c("black","red"))+
...
RelContrib_plot+ylab(bquote("Proportional contribution to ("~ sum(E[v]^sigma)~" )"))
```

#### Raw figure

Provide the raw figure for posterior editing in proper software

```{r fig.height=13, fig.width=13, message=FALSE, warning=FALSE}
cowplot::plot_grid(
  plotbuff_base+ylab(expression(paste("Vital rate elasticity ( ",E[v]," )"))),
  RelContrib_plot+ylab(bquote("Proportional contribution to ("~ sum(E[v]^sigma)~" )")),
    ncol=1,labels="AUTO")
...
[^1]: Remember that absolute msum of stochastic elasticity within
    respect to variance $| \sum E_v^\sigma |$ is extremaly important to
    the analyses because our question is about the demographic position,
    not what determine the contribution of these vital rates. If we use
    raw values we could answers other question but not their position
    about buffering capacity
